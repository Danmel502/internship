...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        $description = trim($_POST['system_description'] ?? '');
        $isActive = isset($_POST['is_active']) ? 1 : 0;
        
        if (!empty($name)) {
            try {
                $db = Database::getInstance()->getDatabase();
                $collection = $db->getCollection('system_names');
        ...
        if (!empty($id) && !empty($name)) {
            try {
                ...
                $currentSystem = $collection->findOne(['id' => (int)$id]);
                if (!$currentSystem) {
                    $message = 'System name not found';
                    $messageType = 'error';
                } else {
                    // Check if this system has dependencies
                    $hasDependencies = $overallCollection->countDocuments(['system_name_id' => (int)$id]) > 0;
                    
                    // Only check dependencies if trying to disable an active system
                    if ($hasDependencies && $currentSystem['is_active'] && !$isActive) {
                        $message = 'Cannot disable system name - it has dependencies';
                        $messageType = 'error';
                    } else {
            ...
            $message = 'System ID and name are required';
            $messageType = 'error';
        }
    }
}
...
try {
    ...
    foreach ($cursor as $doc) {
        ...
        $hasDependencies = $overallCollection->countDocuments(['system_name_id' => $doc['id']]) > 0;
...
<head>
    ...
    <title>System Names Management - Media Track</title>
...
<body class="bg-light">
                ...
                <ul class="navbar-nav ms-auto">
                    ...
                    <li class="nav-item dropdown" id="settingsDropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end" id="settingsDropdownMenu">
                            <li><a class="dropdown-item active" href="system-names.php">System Names</a></li>
    ...
    <section class="hero py-5 text-center text-white">
        <div class="container">
            <h1 class="fw-bold display-6">System Names Management</h1>
            <p class="lead mt-3">
                Manage and configure system names for your media tracking application.<br>
                Create, update, and organize system configurations with ease.
            </p>
        </div>
    </section>
    ...
    <section class="py-5 bg-white">
        <div class="container">
            ...
            <!-- Add New System Name Section -->
            <div class="content-box p-4 p-md-5 mb-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h3 class="mb-0">Add New System Name</h3>
                ...
                <form method="POST" id="addSystemForm">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-12">
                            <label for="systemName" class="form-label">System Name <span class="text-danger">*</span></label>
                            <input type="text" name="system_name" id="systemName" class="form-control mb-3" 
                                   placeholder="Enter system name" required>
                            
                            <label for="systemDescription" class="form-label">Description</label>
                            <textarea name="system_description" id="systemDescription" class="form-control mb-3" 
                                      rows="3" placeholder="Enter system description (optional)"></textarea>
                    ...
                    <div class="row">
                        <div class="col-md-12 text-end">
                            <button type="submit" class="btn btn-success">
                                <i class="fas fa-plus me-2"></i>Add System Name
                            </button>
            ...
            <div class="content-box p-4 p-md-5">
                <h3 class="mb-4">System Names List</h3>
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-striped">
                        ...
                        <tbody>
                            <?php if (empty($systemNames)): ?>
                                <tr>
                                    <td colspan="7" class="text-center text-muted py-4">
                                        <p class="mb-0">No system names found</p>
                                    </td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($systemNames as $system): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php if ($system['has_dependencies']): ?>
                                        ...
                                        <td>
                                            <div class="small">
                                                <div><strong>Created:</strong> 
                                                    <?php echo $system['created_at'] ? date('M d, Y H:i', $system['created_at']->toDateTime()->getTimestamp()) : 'N/A'; ?>
                                                </div>
                                                <?php if ($system['updated_at'] && $system['updated_at'] != $system['created_at']): ?>
                                                <div class="text-muted"><strong>Updated:</strong> 
                                                    <?php echo date('M d, Y H:i', $system['updated_at']->toDateTime()->getTimestamp()); ?>
                                                </div>
                                                <?php endif; ?>
                                            </div>
                                        </td>
                                        <td>
                                            <button type="button" class="btn btn-sm btn-outline-success" 
                                                    onclick="editSystem(<?php echo $system['id']; ?>, '<?php echo htmlspecialchars($system['name'], ENT_QUOTES); ?>', '<?php echo htmlspecialchars($system['description'], ENT_QUOTES); ?>', <?php echo $system['is_active'] ? 'true' : 'false'; ?>, <?php echo $system['has_dependencies'] ? 'true' : 'false'; ?>)">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editModalLabel">Edit System Name</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form method="POST" id="editSystemForm">
                    <div class="modal-body">
                        <input type="hidden" name="action" value="update">
                        <input type="hidden" name="system_id" id="editSystemId">
                        
                        <div class="mb-3">
                            <label for="editSystemName" class="form-label">System Name <span class="text-danger">*</span></label>
                            <input type="text" name="system_name" id="editSystemName" class="form-control" 
                                   placeholder="Enter system name" required>
                        </div>
                        
                        <div class="mb-3">
                            <label for="editSystemDescription" class="form-label">Description</label>
                            <textarea name="system_description" id="editSystemDescription" class="form-control" 
                                      rows="3" placeholder="Enter system description (optional)"></textarea>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div id="editDependencyWarning" class="text-warning mt-1 small" style="display: none;">
                                ...
                                Cannot disable - this system name has dependencies
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-success">
                            <i class="fas fa-save me-2"></i>Update System Name
                        </button>
                    </div>
                </form>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let currentSystemHasDependencies = false;
        let currentSystemIsActive = false;

        if (editIsActiveCheckbox && editDependencyWarning) {
            // Prevent disabling when system has dependencies and is currently active
            editIsActiveCheckbox.addEventListener('change', function(e) {
                if (currentSystemHasDependencies && currentSystemIsActive && !this.checked) {
                    // Prevent unchecking if system has dependencies and is currently active
                    e.preventDefault();
                    this.checked = true;
                    return false;
                }
    ...
    function editSystem(id, name, description, isActive, hasDependencies) {
        ...
        currentSystemHasDependencies = hasDependencies;
        currentSystemIsActive = isActive;
